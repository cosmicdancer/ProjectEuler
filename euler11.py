...
# Check all horizontal sets
for y in range(0,19):
    for x in range(0,15):
        if product(matrix[y][x:x+4]) > greatestproduct:
            greatestproduct = product(matrix[y][x:x+4])

# Check all vertical sets
for x in range(0,19):
    for y in range(0,16):
        set = []
        set.append(matrix[y][x])
        set.append(matrix[y+1][x])
        set.append(matrix[y+2][x])
        set.append(matrix[y+3][x])
        if product(set) > greatestproduct:
            greatestproduct = product(set)
...
for x in range(0,16):
    for y in range(0,16):
        set = []
        set.append(matrix[y][x])
        set.append(matrix[y+1][x+1])
        set.append(matrix[y+2][x+2])
        set.append(matrix[y+3][x+3])
        if product(set) > greatestproduct:
            greatestproduct = product(set)
